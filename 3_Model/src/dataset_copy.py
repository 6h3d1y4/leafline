...
Augmentation:
  - RGBI (ch 0-3) gets randomly adjusted brightness (*0.8-1.2) and contrast (*0.8-1.2)
  - NDVI (ch 4) is recalculated after adjustments
  - nDOM (ch 5) is left unchanged
  - image is randomly flipped vertically and/or horizontally
...
def _augment(
    ...
    """
    Independently apply augmentations to an image patch.

    Augmentations (each applied with probability p):
      - Brightness (channels 0-4)
      - Contrast (channels 0-4)
      - Horizontal flip
      - Vertical flip

    Channels:
      0-3 : RGBI
      4   : NDVI (recalculated after augmentation)
      5   : nDOM (left unchanged by spectral augmentations)
    """

    # ----- Brightness -----
    if torch.rand(1).item() < p: #if random number within probability, augment with this
        brightness = torch.empty(1).uniform_(0.8, 1.2).item() #augment brightness
        r_patch[:5] = (r_patch[:4] * brightness).clamp(0.0, 1.0) #only apply to RGBI

    # ----- Contrast -----
    if torch.rand(1).item() < p:
        contrast = torch.empty(1).uniform_(0.8, 1.2).item()
        r_patch[:5] = ((r_patch[:4] - 0.5) * contrast + 0.5).clamp(0.0, 1.0)
    ...
    ndvi = (aug_nir - aug_red) / (aug_nir + aug_red + eps)
    ...
    if torch.rand(1).item() < p:
        ...
        t_patch = torch.flip(t_patch, dims=[2]) #also apply flip to ground truth

    # ----- Vertical flip -----
    if torch.rand(1).item() < p:
        r_patch = torch.flip(r_patch, dims=[1])
        t_patch = torch.flip(t_patch, dims=[1]) #also apply flip to ground truth

    return r_patch, t_patch
